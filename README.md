...
- **Data flow / instant discovery**: `public/papers.json` ships a **baked `frontier`** — the seed
  set's Semantic Scholar recommendations as small vector-less candidate objects (~30KB) — so the
  discovery slider reveals its first paper **instantly on page load**, straight from the static file
  with zero network wait (each subsequent tick is a synchronous ~0.3ms render). On load the page then
  **refreshes that frontier in the background** (`loadFrontier(false, true)`) to replace the baked
  snapshot with fresh, embedding-ranked candidates — silently, no ghosts/status until you drag.
  Adding a paper or expanding the frontier also calls `api/paper.js` — a root-level **Vercel
  serverless function** that proxies Semantic Scholar and holds the API key; if it's unavailable
  (e.g. `astro dev`) the client falls back to calling Semantic Scholar directly (anonymous,
  CORS-enabled). The baked frontier is regenerated by `scripts/seed-papers.mjs` alongside the nodes.
  *(Trade-off: the background refresh means a page view makes one recommendations + one embeddings
  request, where before a pure view made none.)*
